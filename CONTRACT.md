...
|---|---|---|
| `packages/index/src/serverless.mjs` (via `api/discovery/*.mjs`) | `sextants.dev` | yes |
...
`rec.resource.url`. Until both land, do not assume `:4022` and `sextants.dev` agree.
...
```js
{
  ...
  // ---- additive, SEXTANT-native: not spec, ignored by a spec consumer ----
