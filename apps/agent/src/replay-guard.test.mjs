...
test('a replayed PAYMENT-SIGNATURE header is rejected with a non-null reason', async () => {
  ...
  try {
    ...
    assert.equal(replay.code, ERROR_CODES.SEXTANT_REPLAY_REJECTED);
...
test('an expired authorization entry is rejected with a non-null reason', async () => {
  ...
  try {
    ...
    assert.equal(res.code, ERROR_CODES.SEXTANT_AUTH_EXPIRED);
...
test('every rejection carries a non-null reason and a known code', async () => {
  ...
  for (const bad of [null, undefined, '', '   ']) {
    const f = fail('SEXTANT_SETTLE_FAILED', bad);
  ...
  assert.equal(classifySettleFailure('payment replayed'), 'SEXTANT_REPLAY_REJECTED');
  assert.equal(classifySettleFailure('nonce already used'), 'SEXTANT_REPLAY_REJECTED');
  assert.equal(classifySettleFailure('auth entry expired'), 'SEXTANT_AUTH_EXPIRED');
  assert.equal(classifySettleFailure('ledger bounds exceeded'), 'SEXTANT_AUTH_EXPIRED');
  assert.equal(classifySettleFailure('insufficient balance'), 'SEXTANT_INSUFFICIENT_BALANCE');
  assert.equal(classifySettleFailure('something unheard of'), 'SEXTANT_SETTLE_FAILED');
  assert.equal(classifySettleFailure(undefined), 'SEXTANT_SETTLE_FAILED');

  // Real client rejections, no server involved.
  const noUrl = await payAndFetch('', { config: HERMETIC });
  assert.equal(noUrl.ok, false);
  assert.equal(noUrl.code, ERROR_CODES.SEXTANT_BAD_REQUEST);
  assert.ok(noUrl.reason);

  const noKey = await payAndFetch('http://127.0.0.1:1/x', { config: { PAYER_SECRET: '' } });
  assert.equal(noKey.ok, false);
  assert.equal(noKey.code, ERROR_CODES.SEXTANT_CONFIG_MISSING);
  assert.ok(noKey.reason);

  const unreachable = await payAndFetch('http://127.0.0.1:1/x', { config: HERMETIC, timeoutMs: 1500 });
  assert.equal(unreachable.ok, false);
  assert.ok([ERROR_CODES.SEXTANT_RESOURCE_UNREACHABLE, ERROR_CODES.SEXTANT_TIMEOUT].includes(unreachable.code));
  assert.ok(unreachable.reason);

  // Every code in the enum is SEXTANT-namespaced.
  for (const [k, v] of Object.entries(ERROR_CODES)) {
    assert.equal(k, v);
    assert.match(v, /^SEXTANT_[A-Z0-9_]+$/);
  }
});
