...
function writeEnv(vals) {
  const body = `# SEXTANT — generated by scripts/setup-testnet.mjs
...
function writeTxDoc() {
  ...
  const rows = txs
    ...
    .join("\n");
  const header = `# SEXTANT — testnet transactions
...
async function main() {
  log("\nSEXTANT testnet setup — no faucets, no captchas, no API keys.\n");
  ...
  await submitClassic("Issuer setOptions (home domain)", issuer.kp, (b) =>
    b.addOperation(Operation.setOptions({ homeDomain: "sextant.local" })),
  );
