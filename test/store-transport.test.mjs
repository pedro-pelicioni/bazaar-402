 ...
 *                 `error` event.
 *
 * The end-to-end round trip against a real server runs only when SEXTANT_TEST_REDIS_URL
...
test('discretion: a failed protocol command never leaks the password', async () => {
  ...
  const store = createStore({
    ...
    SEXTANT_REDIS_CONNECT_TIMEOUT_MS: '250',
  });
...
test('degradation: an unreachable protocol store answers, it does not throw or hang', async () => {
  const env = { KV_REDIS_URL: 'redis://127.0.0.1:1', SEXTANT_REDIS_CONNECT_TIMEOUT_MS: '250' };
...
const LIVE_URL = process.env.SEXTANT_TEST_REDIS_URL;

test(
  'redis: a real server round-trips a record and reuses one connection',
  { skip: LIVE_URL ? false : 'set SEXTANT_TEST_REDIS_URL to run against a real Redis' },
  async () => {
    const env = { KV_REDIS_URL: LIVE_URL, SEXTANT_KV_KEY: `sextant:test:${Date.now()}` };
    const store = createStore(env);
    try {
...
test(
  ...
  async () => {
    ...
    const key = `sextant:test:wrongtype:${Date.now()}`;
    const clash = createStore({ KV_REDIS_URL: LIVE_URL, SEXTANT_KV_KEY: key });
    try {
      const store = createStore({ KV_REDIS_URL: LIVE_URL, SEXTANT_KV_KEY: `${key}:ok` });
