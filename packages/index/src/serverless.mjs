...
function cacheControl(env = process.env) {
  const sMaxAge = Number.parseInt(env?.SEXTANT_CACHE_S_MAXAGE ?? '', 10);
  const swr = Number.parseInt(env?.SEXTANT_CACHE_SWR ?? '', 10);
...
function storeTtlMs(env = process.env) {
  const ttl = Number.parseInt(env?.SEXTANT_KV_TTL_MS ?? '', 10);
 ...
 * SEXTANT_WRITE_TOKEN makes them *permitted*, and the absence of either is reported
 * plainly instead of silently accepted.
 */
function writeState(state, env = process.env) {
  const token = typeof env?.SEXTANT_WRITE_TOKEN === 'string' ? env.SEXTANT_WRITE_TOKEN.trim() : '';
  ...
  if (!token) {
    return {
      enabled: false,
      reason:
        'catalog is read-only: a durable store is configured but SEXTANT_WRITE_TOKEN is unset, and an unauthenticated public write endpoint is refused by design.',
    };
  }
...
async function upsertResource(req, res, env = process.env) {
  ...
  if (!tokenMatches(bearerOf(req), write.token)) {
    return sendJson(
      res,
      401,
      { ok: false, dropped: [], reason: 'a valid `Authorization: Bearer <SEXTANT_WRITE_TOKEN>` header is required' },
